#!/usr/bin/env python3
"""
TD3 测试脚本 - 使用 SCG 精确奖励（与训练脚本/Pi-Flight/传统控制器对齐）
所有参数写在脚本顶部，直接修改即可
...
# --------------------- 任务配置 ---------------------
TASK = "figure8"          # 选择: circle, square, helix, figure8 (必须与训练时一致)
DURATION = 5.0            # 每个episode时长(秒)

# --------------------- 测试配置 ---------------------
NUM_ENVS = 1024           # 并行环境数 (与 Pi-Flight/CPID 评估对齐)
EPISODES = 10             # 测试episode数
DEVICE = "cuda:0"         # 设备

# --------------------- 模型路径 ---------------------
# 修改为你的模型路径
MODEL_PATH = ROOT / "04_TD3" / "results" / "checkpoints" / TASK / "td3_final.zip"

# --------------------- 输出配置 ---------------------
OUTPUT_PATH = ROOT / "04_TD3" / "results" / f"eval_{TASK}.json"
...
def main():
    ...
    if not MODEL_PATH.exists():
        ...
        print("   请先运行 train_td3.py 训练模型，或修改 MODEL_PATH 指向正确的模型文件")
        return
