...
class OnlineTrainer:
    ...
    def __init__(self, args):
        ...
        print(
            "[Trainer] MCTS参数已封闭: exploration_weight=2.5, puct_c=1.5, max_depth=12, "
    ...
    def mcts_search(self, root_program: List[Dict[str, Any]], num_simulations: int = 800, iter_idx: int = 0) -> Tuple[List[Any], List[int]]:
        ...
        if curriculum_ops:  # 限制算子集合
            allowed_ops_full = ['+','-','*','/','max','min','abs','sqrt','log1p','>','<']
            dsl_operators = [op for op in allowed_ops_full if op in curriculum_ops]
        else:
    ...
    def train(self):
        ...
        # 🔒 最终保存：使用深拷贝的best_program_copy,确保不被cleanup影响
...
def parse_args(argv=None):
    ...
    p.add_argument('--program-history-path', type=str, default='01_pi_flight/results/program_history.jsonl', help='保存程序演化历史(JSON Lines)，仅在出现新best时追加')
    # 调试/诊断
