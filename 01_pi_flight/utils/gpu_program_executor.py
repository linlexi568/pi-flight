"""GPU 表达式执行器：支持 PI-Flight DSL 的 Level 1/2/3 GPU 求值。
    ...
    def compile(self, program: List[Dict[str, Any]]) -> CompiledProgram:
        ...
        for rule in program or []:
            ...
            for action in rule.get("action", []) or []:
                ...
                prev = outputs.get(key)
                if prev is None:
                    outputs[key] = expr_idx
                else:
                    outputs[key] = self._emit_binary("+", prev, expr_idx)
    ...
    def evaluate(
        ...
        *,
        ...
        for key, bucket in grouped.items():
            ...
            outputs[idx_tensor, 0] = torch.clamp(group_outputs[:, 0], -5.0, 5.0)
            outputs[idx_tensor, 1] = torch.clamp(group_outputs[:, 1], -0.02, 0.02)
            outputs[idx_tensor, 2] = torch.clamp(group_outputs[:, 2], -0.02, 0.02)
            outputs[idx_tensor, 3] = torch.clamp(group_outputs[:, 3], -0.01, 0.01)

        for idx in fallback_indices:
            vec = self._run_fallback(programs[idx], state_tensors, idx, node_states[idx])
            outputs[idx, 0] = torch.clamp(vec[0], -5.0, 5.0)
            outputs[idx, 1] = torch.clamp(vec[1], -0.02, 0.02)
            outputs[idx, 2] = torch.clamp(vec[2], -0.02, 0.02)
            outputs[idx, 3] = torch.clamp(vec[3], -0.01, 0.01)
        return outputs
