#!/usr/bin/env python3
"""
PID/CPID/LQR 基线评估脚本 - 使用 SCG 精确奖励（与 Pi-Flight 完全对齐）
所有参数写在脚本顶部，直接修改即可
...
# --------------------- 评估配置 ---------------------
NUM_ENVS = 1024           # 并行环境数 (与 Pi-Flight 评估对齐)
...
PARAMS_FILE = ROOT / "results" / "aligned_baselines" / f"{CONTROLLER}_{TASK}.json"
# 如果文件不存在，将使用下面的默认参数
...
# --------------------- 输出配置 ---------------------
OUTPUT_PATH = ROOT / "results" / "aligned_baselines" / f"eval_{CONTROLLER}_{TASK}.json"
