#!/usr/bin/env python3
"""Compare PID / LQR / π-Flight controllers inside the Isaac tester with SCG rewards."""
...
for candidate in (ROOT, ROOT / "scripts", ROOT / "01_pi_flight", ROOT / "utilities"):
...
class DirectForcePiFlightWrapper:
    """
    直接输出 6D 力/力矩的 π-Flight 包装器，与 BatchEvaluator 的训练方式一致。
...
def build_piflight_controller(program_path: Path) -> DirectForcePiFlightWrapper:
    program = load_program(program_path)
    return DirectForcePiFlightWrapper(program=program)

def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run PID/LQR/π-Flight inside Isaac tester with SCG reward")
    ...
    parser.add_argument("--output", type=str, default="results/isaac_compare_pid_lqr_piflight.json")
...
def main() -> None:
    ...
    piflight_ctrl = build_piflight_controller(program_path)

    results = []
    results.append(run_controller("PID", pid_ctrl, args))
    results.append(run_controller("LQR", lqr_ctrl, args))
    results.append(run_controller("PiFlight", piflight_ctrl, args))
