...
使用示例:
    ...
    python scripts/sb3/compare_eval.py \
        ...
        --piflight-program results/figure8-safe_control_tracking.json \
        --trajectory figure8 \
        --episodes 20
"""
...
from scripts.sb3.piflight_policy import PiFlightPolicy, load_piflight_program
...
def evaluate_policy(
    ...
    Args:
        policy: 策略对象（SB3 模型或 PiFlightPolicy）
        env: 评估环境
...
def compare_methods(
    ...
    # 评估 π-Flight 程序
    if piflight_program_path:
        try:
            print(f"\n📦 加载 π-Flight 程序: {piflight_program_path}")
            program = load_piflight_program(piflight_program_path)
            policy = PiFlightPolicy(program, dt=1.0/50.0)
            
            print(f"🔄 评估 π-Flight 程序...")
            piflight_results = evaluate_policy(
                policy, env, n_episodes,
                policy_name="π-Flight",
                is_sb3=False,
            )
            results['methods']['piflight'] = piflight_results
            
        except Exception as e:
            print(f"⚠️ π-Flight 程序加载失败: {e}")
...
def main():
    parser = argparse.ArgumentParser(description='对比评估 SB3 和 π-Flight')
    
    parser.add_argument('--sb3-model', type=str, help='SB3 模型路径')
    parser.add_argument('--piflight-program', type=str, help='π-Flight 程序路径')
