...
模块结构:
    ...
    - piflight_policy.py: 将 π-Flight DSL 程序转换为 SB3 兼容策略
    - compare_eval.py: 对比评估 SB3 模型和 π-Flight 程序
    - isaac_gym_wrapper.py: Isaac Gym 的 SB3 向量化环境封装

使用流程:
    1. 训练 π-Flight: ./run.sh (使用 sb3_standard reward profile)
    2. 训练 SB3 baseline: python scripts/sb3/train_sb3.py --algo ppo
    3. 对比评估: python scripts/sb3/compare_eval.py --sb3-model ... --piflight-program ...
...
快速开始:
    ...
    # 2. 在 SB3 环境中测试 π-Flight 程序
    from scripts.sb3.piflight_policy import PiFlightPolicy, evaluate_piflight_in_sb3
    
    results = evaluate_piflight_in_sb3(
    ...
    compare_methods(
        ...
        piflight_program_path='results/figure8-sb3_standard.json',
        trajectory='figure8',
    )
    ```
...
Reward 设计:
    ...
    对应的 π-Flight reward profile: sb3_standard
...
from scripts.sb3.piflight_policy import (
    PiFlightPolicy,
    load_piflight_program,
    evaluate_piflight_in_sb3,
)
