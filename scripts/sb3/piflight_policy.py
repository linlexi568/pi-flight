"""π-Flight 程序适配器：将 DSL 程序转换为 SB3 兼容的策略。

这个模块允许在 SB3 环境中测试 π-Flight 生成的控制程序。
...
class PiFlightPolicy:
    """将 π-Flight DSL 程序转换为 SB3 兼容的策略。
    ...
    def __init__(
        ...
        """
        Args:
            program: π-Flight DSL 程序（列表或 JSON 文件路径）
            dt: 控制周期（用于积分项计算）
        """
        # 加载程序
...
def load_piflight_program(path: Union[str, Path]) -> List[Dict]:
    """加载 π-Flight 程序。
...
def evaluate_piflight_in_sb3(
    program_path: Union[str, Path],
    trajectory: str = 'figure8',
    n_episodes: int = 10,
    render: bool = False,
) -> Dict[str, Any]:
    """在 SB3 环境中评估 π-Flight 程序。
    ...
    # 加载程序
    program = load_piflight_program(program_path)
    policy = PiFlightPolicy(program)
    ...
    print(f"\n📊 π-Flight 评估结果 ({n_episodes} episodes):")
