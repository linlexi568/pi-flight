...
本目录提供 Stable-Baselines3 (SB3) 与 π-Flight 的完整集成。
...
├── piflight_policy.py   # π-Flight 程序适配器
...
1. **训练**: Isaac Gym (GPU 加速) + π-Flight DSL 程序搜索
...
### 3. 使用 π-Flight 训练（SB3 对齐 reward）
...
python scripts/sb3/compare_eval.py \
    ...
    --piflight-program results/figure8-sb3_standard.json \
    --trajectory figure8 \
    --episodes 20
```
...
# 从 JSON 文件加载
policy = PiFlightPolicy('results/best_program.json')
...
```python
from scripts.sb3.piflight_policy import evaluate_piflight_in_sb3
...
3. **积分状态**: `PiFlightPolicy` 内部维护积分状态，每个 episode 需调用 `reset()`。
