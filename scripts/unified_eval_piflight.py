#!/usr/bin/env python3
"""
统一评估脚本：用完全相同的配置重新评估 Pi-Flight 和传统控制器
确保奖励计算方式一致
"""
import sys
import json
...
try:
    ...
    program = piflight_data['rules']
    print(f"\n  程序规则数: {len(program)}")
    print(f"  训练时奖励: {piflight_data['meta']['reward']:.2f}")
    ...
    print(f"\n  对比:")
    print(f"    训练时: {piflight_data['meta']['reward']:.2f}")
    print(f"    重新评估: {reward_true:.2f}")
    print(f"    差异: {abs(reward_true - piflight_data['meta']['reward']):.2f}")
