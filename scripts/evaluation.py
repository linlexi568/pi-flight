...
使用方式:
    ...
    # 评估 π-Flight 程序
    piflight_results = evaluator.evaluate_piflight_program('results/best_program.json')
    
    # 生成对比表格
    evaluator.generate_comparison_table([sb3_results, piflight_results])
"""
...
sys.path.insert(0, str(_project_root / "01_pi_flight"))
...
class StandardEvaluator:
    ...
    def evaluate_piflight_program(
        ...
        """
        try:
            from scripts.sb3.piflight_policy import PiFlightPolicy
            from scripts.sb3.quadrotor_env import QuadrotorTrackingEnv
        except ImportError as e:
            print(f"❌ 导入失败: {e}")
            return EvalMetrics(method_name="π-Flight")
        
        # 加载程序
        policy = PiFlightPolicy(program_path)
        ...
        metrics = self._aggregate_results(all_results, "π-Flight")
        return metrics
...
def run_full_evaluation(
    ...
    piflight_program_path: Optional[str] = None,
    ...
    Args:
        ...
        piflight_program_path: π-Flight 程序路径
    ...
    # 评估 π-Flight 程序
    if piflight_program_path and os.path.exists(piflight_program_path):
        print(f"\n📊 评估 π-Flight 程序: {piflight_program_path}")
        piflight_results = evaluator.evaluate_piflight_program(piflight_program_path)
        results.append(piflight_results)
        print(f"   Position RMSE: {piflight_results.position_rmse:.4f} ± {piflight_results.position_rmse_std:.4f}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="π-Flight 标准评估框架")
    ...
    parser.add_argument("--sb3-model", type=str, default=None,
                        help="SB3 模型路径")
    parser.add_argument("--piflight-program", type=str, default=None,
                        help="π-Flight 程序路径")
    ...
    if args.citations:
        ...
        run_full_evaluation(
            ...
            piflight_program_path=args.piflight_program,
