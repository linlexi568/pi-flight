...
# Run with the project venv python: /home/linlexi/桌面/pi-flight/.venv/bin/python scripts/smoke_hover.py
...
try:
    # Create a synthetic package 'piflight' so relative imports (from .dsl) work
    # when loading modules via file paths from '01_pi_flight'.
    if 'piflight' not in sys.modules:
        pkg = types.ModuleType('piflight')
        pkg.__path__ = [os.path.join(root, '01_pi_flight')]
