...
# -*- coding: utf-8 -*-
"""
Minimal evaluation runner: safe-control-gym × Pi-Flight adapter
...
Optional (using Pi-Flight DSL program):
    python scripts/eval_safecontrol_piflight.py \
        --env-id quadrotor_tracking \
        --episodes 3 \
        --controller piflight \
...
from adapters.safecontrol_piflight_adapter import PiFlightPolicyAdapter

CONTROLLER_CHOICES = ('pid', 'lqr', 'piflight')
...
def evaluate(env_id: str, episodes: int, controller: str, action_space: str, program_json: str, device: str, seed: int):
    ...
    adapter = PiFlightPolicyAdapter(
        ...
        program_json=program_json if controller == 'piflight' else None,
        device=device,
...
def main():
    ...
    # Friendly checks
    if args.controller == 'piflight' and not args.program_json:
        print('[Warn] --controller piflight specified but --program-json is empty; falling back to PID baseline.')
        args.controller = 'pid'
    try:
        evaluate(
            env_id=args.env_id,
