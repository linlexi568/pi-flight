...
评估 Pi-Flight / SAC / TD3 / PID / CPID / LQR 在同一任务上的表现
...
if str(ROOT / '01_pi_flight') not in sys.path:
...
# --------------------- 要评估的方法 ---------------------
# True = 评估该方法, False = 跳过
EVAL_PIFLIGHT = True      # Pi-Flight DSL 程序
...
# --------------------- 模型/程序路径 ---------------------
PIFLIGHT_PATH = ROOT / "results" / "scg_aligned" / f"{TASK}_safe_control_tracking_best.json"
...
def eval_piflight(program_path, task, duration, num_envs):
    """评估 Pi-Flight 程序"""
    import json
...
def main():
    ...
    # 1. Pi-Flight
    if EVAL_PIFLIGHT:
        print("[1/6] 评估 Pi-Flight...")
        pf_result = eval_piflight(PIFLIGHT_PATH, TASK, DURATION, NUM_ENVS)
        if pf_result:
            results['methods']['Pi-Flight'] = pf_result
            print(f"      ✓ 奖励: {pf_result['mean_reward']:.2f}")
        else:
            print(f"      ✗ 程序不存在: {PIFLIGHT_PATH}")
