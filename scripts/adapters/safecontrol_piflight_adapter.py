...
# -*- coding: utf-8 -*-
"""
Safe-Control-Gym × Pi-Flight Adapter (skeleton)

Purpose
- Wrap Pi-Flight controller (DSL or baseline PD) into a callable policy for safe-control-gym
...
Quick use
- As a library: from adapters.safecontrol_piflight_adapter import PiFlightPolicyAdapter
- In eval script: adapter = PiFlightPolicyAdapter(mode='pid', action_space='thrust_torque')

Notes
- This file avoids hard dependency on Pi-Flight internals at import time; actual import is lazy.
- When mode='piflight', it will attempt to load a JSON program via compare_gpu_cpu_executor._load_programs.
"""
...
# Adapter
# ------------------------------
class PiFlightPolicyAdapter:
    """
    A callable policy that maps safe-control-gym observations to actions using:
    - mode='piflight': Pi-Flight DSL program via GPUProgramExecutor (if available)
    ...
    def __init__(
        ...
        assert mode in ('pid', 'lqr', 'piflight')
        self.mode = mode
        ...
        if self.mode == 'piflight':
            self._lazy_init_piflight()
    ...
    # --------- Pi-Flight lazy init ---------
    def _lazy_init_piflight(self) -> None:
        # Inject repo paths
        repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
        sys.path.insert(0, os.path.join(repo_root, '01_pi_flight'))
        ...
        if not programs:
            ...
            self.mode = 'pid'
            return
        self._piflight = self._executor_cls(device=self._device)
        self._piflight.reset_state()
        self._token = self._piflight.prepare_batch(programs)
    ...
    # --------- Pi-Flight DSL path ---------
    def _act_piflight(self, st: Dict[str, float]) -> Tuple[float, float, float, float]:
        if not self._piflight or not self._token:
        ...
        (outputs, _, _) = self._piflight.evaluate_from_raw_obs(
    ...
    def act(self, obs: Any) -> np.ndarray:
        ...
        if self.mode == 'piflight':
            u_fz, u_tx, u_ty, u_tz = self._act_piflight(st)
        elif self.mode == 'lqr':
    ...
    def close(self) -> None:
        try:
            if self._piflight and self._token is not None:
                self._piflight.release_batch(self._token)
        except Exception:
            pass
        self._piflight = None
        self._token = None
...
if __name__ == '__main__':
    adapter = PiFlightPolicyAdapter(mode='pid', action_space='thrust_torque')
    dummy_obs = {
        'pos': [0.2, -0.1, -0.05],
        'vel': [0.0, 0.0, 0.0],
        'rpy': [0.05, -0.04, 0.02],
