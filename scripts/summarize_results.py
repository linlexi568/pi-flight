...
# 各方法的结果文件路径
RESULT_FILES = {
    'Pi-Flight': ROOT / "results" / "piflight" / f"eval_{TASK}.json",
...
def main():
    ...
    if not results:
        print("❌ 没有找到任何评估结果文件")
        print("请先运行各个评估脚本:")
        print("  .venv/bin/python 01_pi_flight/eval_piflight.py")
    ...
    # 计算相对提升
    if 'Pi-Flight' in results and len(results) > 1:
        pf_reward = results['Pi-Flight']['reward']
        print("\nPi-Flight 相对于其他方法的提升:")
