...
This package hosts an experimental meta-RL / recurrent policy module that learns to adjust π-Flight's MCTS and evaluator hyper-parameters online. The design goals are:
...
2. **Hot-plug friendly** – the controller can be enabled/disabled without touching `01_pi_flight/train_online.py`. Overrides are written to a JSON file; the trainer only needs to read it before each meta-epoch. When the file is missing, default behavior is unchanged.
