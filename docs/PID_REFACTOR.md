# Pi-Flight PID 封装清理重构说明

## 重构时间
2025年12月5日

## 重构原因
Pi-Flight 已完全迁移到直接输出 `u_fz/u_tx/u_ty/u_tz`（力/力矩）的控制模式，不再需要传统的 PID 封装层。所有 DSL 程序现在直接生成控制指令，无需通过 PID 增益调节。

## 删除的核心文件

### 1. PID 封装核心
- `01_pi_flight/utils/local_pid.py` - SimplePIDControl 基类（已废弃）
- `01_pi_flight/utils/segmented_controller.py` - PiLightSegmentedPIDController（已废弃）
...
- `scripts/compare_simplified_pid_lqr_piflight.py` - 简化版对比脚本
...
python utilities/verify_program.py \
  --program results/piflight_train/square_safe_control_tracking_best.json \
  --traj square \
  --duration 5 \
  --num-envs 1024 \
  --replicas 1
...
**功能**: 对比 PID/CPID/LQR 基线 vs Pi-Flight 程序
...
python utilities/compare_baselines.py \
  --piflight-program results/piflight_train/square_safe_control_tracking_best.json \
  --baseline all \
  --traj square \
  --num-envs 1024
```
...
### `01_pi_flight/utils/batch_evaluation.py`
...
**影响的外部代码**:
- 任何直接导入 `from 01_pi_flight.utils.segmented_controller import ...` 的代码
- 任何直接导入 `from 01_pi_flight import PiLightSegmentedPIDController` 的代码
...
本次重构彻底移除了 Pi-Flight 中的 PID 封装层，简化了系统架构，使其更符合"直接输出控制指令"的设计理念。所有功能已通过新的验证脚本测试，系统运行正常。
