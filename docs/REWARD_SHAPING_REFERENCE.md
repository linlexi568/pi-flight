# π-Flight 奖励塑形与控制理论设计文档

## 1. 概述

本文档详细说明 π-Flight 框架中的奖励函数设计、安全壳配置（Safety Envelope）、以及与经典控制理论的对应关系，旨在为论文撰写、实验设计和系统调优提供理论依据和参考文献索引。

### 1.1 设计理念

π-Flight 采用**多目标奖励塑形（Multi-Objective Reward Shaping）**策略，将无人机控制任务分解为若干可量化的性能指标，每个指标对应控制理论中的经典评价准则。通过加权组合，系统可以在不同应用场景下灵活平衡：
...
**核心目标**：
安全壳（Safety Envelope）是 π-Flight 框架的核心创新之一，通过**物理约束**和**控制理论领域知识**，将无限大的符号程序搜索空间缩小到**物理可实现、控制理论上正确**的子空间，从而：
...
- **π-Flight 安全壳**：显式编码控制理论知识，从源头保证程序的合法性
...
| 方法 | 配置 | 对比维度 |
|------|------|---------|
| **π-Flight (Balanced)** | `balanced` | 综合性能基线 |
| **π-Flight (Safety-First)** | `safety_first` | 安全性、平滑性 |
| **π-Flight (Tracking-First)** | `tracking_first` | 跟踪精度上限 |
| **π-Flight (Robustness)** | `robustness_stability` | 鲁棒性、泛化能力 |
...
- **批量评估器**：`01_pi_flight/utils/batch_evaluation.py`
- **训练脚本**：`01_pi_flight/train_online.py`
- **启动脚本**：`run.sh`
...
**文档维护者**：π-Flight 团队  
**联系方式**：[项目 GitHub](https://github.com/linlexi568/pi-flight)  
**许可证**：MIT License
