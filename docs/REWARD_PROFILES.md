...
π-Flight 项目提供了多套奖励权重配置（Reward Profiles），用于在不同应用场景下引导 MCTS 搜索合成不同风格的控制程序。
...
- 比较对象：PID, PPO, π-Flight (safety_first), π-Flight (tracking_first), π-Flight (balanced)
