...
**π-Flight** 是一个基于 **程序合成 (Program Synthesis)** 和 **强化学习 (Reinforcement Learning)** 的无人机飞行控制律自动生成系统。

不同于传统的端到端强化学习（如 PPO 直接输出电机指令），π-Flight 致力于生成 **可解释、可部署、符合物理约束** 的符号化控制程序（如 PID 变体、自适应控制律）。
...
位于 `01_pi_flight/utils/batch_evaluation.py`。
...
位于 `01_pi_flight/utils/bayesian_tuner.py`。
