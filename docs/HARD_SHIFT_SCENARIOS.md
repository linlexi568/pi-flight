...
- `scenario_kwargs` 允许精细化调参；所有可调字段在 `01_pi_flight/envs/isaac_gym_drone_env.py` 冒头注释中列出。
...
1. **风场 generalization**：`TRAIN_ENV_SCENARIO='default'`，`TEST_ENV_SCENARIO='wind_storm'`，观察 RL vs Pi-Flight 的鲁棒性差异。
