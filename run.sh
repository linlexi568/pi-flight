...
# 训练参数集中配置（在这里改）
# ==============================
TRAIN_TOTAL_ITERS=800
TRAIN_MCTS_SIMS=200
TRAIN_UPDATE_FREQ=999999
TRAIN_STEPS_PER_UPDATE=10
TRAIN_ISAAC_ENVS=8196
TRAIN_MCTS_LEAF_BATCH=1024
...
# 提升MCTS搜索深度，允许生成包含 u_tx/u_ty 的复杂规则
...
STRUCTURE_PRIOR_WEIGHT=1              # 结构先验权重(最大)
STABILITY_PRIOR_WEIGHT=0.8             # 稳定性先验权重(强)
PRIOR_LEVEL=3                          # 最高约束等级
...
SCG_RESULT_DIR="${RESULT_DIR}/piflight_train"
...
PROGRAM_HISTORY_PATH="01_pi_flight/results/program_history.jsonl"
