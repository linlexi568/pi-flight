...
"""
基线控制器对比工具
对比 PID/CPID/LQR 调参结果 vs Pi-Flight DSL 程序
"""
import os
import sys
import json
...
sys.path.insert(0, str(REPO_ROOT / "01_pi_flight"))
...
def main():
    parser = argparse.ArgumentParser(description='对比基线控制器 vs Pi-Flight 程序')
    parser.add_argument('--piflight-program', type=str, help='Pi-Flight 程序路径')
    ...
    # 评估 Pi-Flight 程序
    if args.piflight_program:
        print(f"[1] 评估 Pi-Flight 程序: {args.piflight_program}")
        program, meta = load_program(args.piflight_program)
        pf_results = evaluate_with_batch_evaluator(program, args.traj, args.duration, args.num_envs)
        results['piflight'] = pf_results
    ...
    print("对比结果汇总")
    print("=" * 80)
    print(f"{'方法':<20} {'奖励 (Reward)':<20} {'相对 Pi-Flight':<20}")
    print("-" * 80)
    
    pf_reward = results.get('piflight', {}).get('reward')
    
    for method, res in results.items():
        reward = res.get('reward')
        if reward is not None:
            if pf_reward and method != 'piflight':
