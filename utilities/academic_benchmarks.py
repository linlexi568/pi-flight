...
def build_trajectory(name: str) -> Dict[str, Any]:
    ...
    if name == 'circle':
        return { 'type': 'circle','initial_xyz': [0, 0, 0.9], 'params': {'R': 0.9,'period': 10}}
    if name == 'helix':
        return { 'type': 'helix','initial_xyz': [0, 0, 0.6], 'params': {'R': 0.8,'period': 12,'v_z': 0.12}}
    if name == 'square':
        return { 'type': 'square','initial_xyz': [0, 0, 0.9], 'params': {'side_len': 1.4,'period': 12,'corner_hold': 0.4}}
